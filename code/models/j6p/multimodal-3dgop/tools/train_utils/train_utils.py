...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    argspec = inspect.getfullargspec(lr_scheduler.step)
    num_positional_params = len(argspec.args)
    ckpt_save_cnt = 1
    start_it = accumulated_iter % total_it_each_epoch

    if use_amp:
        scaler = torch.cuda.amp.GradScaler(enabled=use_amp, init_scale=optim_cfg.get('LOSS_SCALE_FP16', 2.0**16))  # torch 1.1 not support
    ...
    if rank == 0:
        ...
        rpn_loss_cls_gop_m = common_utils.AverageMeter()
        rpn_loss_loc_gop_m = common_utils.AverageMeter()
        rpn_loss_cls_m = common_utils.AverageMeter()
        rpn_loss_loc_m = common_utils.AverageMeter()
    end = time.time()
    model.train()
    if cfg is not None and cfg.MODEL.get('FREEZE', None) is not None:  # fix bn: mean, val
        common_utils.fix_modules(model, cfg.MODEL.FREEZE)
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        if use_amp:  # torch 1.1 not support
        ...
        if rank == 0:
            ...
            if gop_flag:
                rpn_loss_loc = tb_dict['loss_dense_head_det_lidar']['rpn_loss_loc']
                rpn_loss_cls = tb_dict['loss_dense_head_det_lidar']['rpn_loss_cls']
                rpn_loss_loc_gop = tb_dict['loss_dense_head_det_lidar_gop']['rpn_loss_loc_gop']
                rpn_loss_cls_gop = tb_dict['loss_dense_head_det_lidar_gop']['rpn_loss_cls_gop']
                rpn_loss_cls_m.update(rpn_loss_cls, batch_size)
                rpn_loss_loc_m.update(rpn_loss_loc, batch_size)
                rpn_loss_cls_gop_m.update(rpn_loss_cls_gop, batch_size)
                rpn_loss_loc_gop_m.update(rpn_loss_loc_gop, batch_size)
                ...
                if accumulated_iter % logger_iter_interval == 0 or cur_it == start_it or cur_it + 1 == total_it_each_epoch:
                    ...
                    if not gop_flag:
                        ...
                        logger.info(
                            'Train: {:>4d}/{} ({:>3.0f}%) [{:>4d}/{} ({:>3.0f}%)]  '
                            'Loss: {loss.val:#.4g} ({loss.avg:#.3g})  '
                            'Loss_pvb_loc: {rpn_loss_loc.val:#.4g} ({rpn_loss_loc.avg:#.3g})  '
                            'Loss_pvb_cls: {rpn_loss_cls.val:#.4g} ({rpn_loss_cls.avg:#.3g})  '
                            'Loss_gop_loc: {rpn_loss_loc_gop.val:#.4g} ({rpn_loss_loc_gop.avg:#.3g})  '
                            'Loss_gop_cls: {rpn_loss_cls_gop.val:#.4g} ({rpn_loss_cls_gop.avg:#.3g})  '
                            'LR: {lr:.3e}  '
                            f'Time cost: {tbar.format_interval(trained_time_each_epoch)}/{tbar.format_interval(remaining_second_each_epoch)} ' 
                            f'[{tbar.format_interval(trained_time_past_all)}/{tbar.format_interval(remaining_second_all)}]  '
                            'Acc_iter {acc_iter:<10d}  '
                            ...
                            'Batch time: {batch_time.val:.2f}({batch_time.avg:.2f})'.format(
                                ...
                                cur_it,total_it_each_epoch, 100. * cur_it / total_it_each_epoch,
                                loss=losses_m,
                                rpn_loss_loc=rpn_loss_loc_m,
                                rpn_loss_cls=rpn_loss_cls_m,
                                rpn_loss_loc_gop=rpn_loss_loc_gop_m,
                                rpn_loss_cls_gop=rpn_loss_cls_gop_m,
                                lr=cur_lr,
                                acc_iter=accumulated_iter,
                                data_time=data_time,
                                forward_time=forward_time,
                ...
                for key, val in tb_dict.items():
                    if type(val) is dict:
                        for key1, val1 in val.items():
                            tb_log.add_scalar('train/' + key + '/' + key1, val1, accumulated_iter)
                    else:
...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
                ...
                merge_all_iters_to_one_epoch=False, use_amp=False, use_logger_to_record=False, logger=None, logger_iter_interval=None, 
                ckpt_save_time_interval=None, show_gpu_stat=False, cfg=None, args=None, test_loader=None):
    ...
    if cfg.DATA_CONFIG.get('TASKS', None) is not None:
        tasks = cfg.DATA_CONFIG.TASKS
        best_miou = {'miou':[0.0 for _ in range(len(tasks))], 'biou':[0.0 for _ in range(len(tasks))]}
    else:
        best_miou = None
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            if cfg.OPTIMIZATION.get('EVAL_WHEN_TRAINING', False) and \
                (trained_epoch >= cfg.OPTIMIZATION.EVAL_START_EPOCH or trained_epoch % 10 == 0):
                output_dir = Path(os.path.split(ckpt_save_dir)[0]) / 'eval_ckpt'
                if args.eval_type == 'seg_det':
                    eval_seg_det_utils.eval_one_epoch(
                        cfg, args, model, test_loader, trained_epoch, logger, result_dir=output_dir, best_miou=best_miou
                    )
                else:
                    eval_utils.eval_one_epoch(
                        cfg, args, model, test_loader, trained_epoch, logger, dist_test=True, result_dir=output_dir
                    )
