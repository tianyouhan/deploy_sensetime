...
class OptimWrapper():
    ...
    def __init__(self, opt, wd, true_wd: bool = False, bn_wd: bool = True, layer_lr=None, fix_layer_lr=False):
        self.opt, self.true_wd, self.bn_wd = opt, true_wd, bn_wd
        self.opt_keys = list(self.opt.param_groups[0].keys())
        self.opt_keys.remove('params')
        self.read_defaults()
        self.wd = wd
        self.layer_lr = layer_lr
        self.fix_layer_lr = fix_layer_lr
    ...
    def lr(self, val: float) -> None:
        if self.layer_lr is not None and type(val) is not list:
            if self.fix_layer_lr:
                lr_list = [ele for ele in self.layer_lr]
                lr_list[-1] = val
            else:
                lr_list = [val * (ele / self.layer_lr[-1]) for ele in self.layer_lr]
        else:
            lr_list = listify(val, self._lr)
        self._lr = self.set_val('lr', lr_list)
