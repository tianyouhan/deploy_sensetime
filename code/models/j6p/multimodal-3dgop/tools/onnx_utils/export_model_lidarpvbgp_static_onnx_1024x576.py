...
def apply_onnx_transfer(batch_dict, module_list, onnx_file_name):
    ...
    with torch.no_grad():
        for idx in range(len(module_list)):
            ...
            if cur_onnx_file_name in ['fuser-fusion-head', 'lidar-head', 'cam-head', 'lidar-headgop']:  # 因为输出是二维字典，而onnx只输出val，因此需要val数量的output_names
...
if __name__ == '__main__':
    ...
    export_fp16 = False
    export_dynamic = False  # J6M adela部署需要静态维度
    dtype = torch.float16 if export_fp16 else torch.float32
    cfg_file = "cfgs/A02_ceph/GAC-P643M1ATX_PVBGOP-J6M_volc_onnx.yaml"
    checkpoint_path = "../output/A02_ceph/GAC-P643M1ATX_PVBGOP-J6M_volc/default/ckpt/checkpoint_epoch_60.pth"  # 在这里指定checkpoint_path
    save_onnx_root = "onnx_utils/checkpoints/GAC-P643M1ATX_PVBGOP-J6M_volc/"
    ...
    module_list = [
        ...
        ['pp_heavy_head_pvbgop'],  # lidarpvb
        ['pp_heavy_head_pvbgop'],  # lidargop
    ]
    onnx_inputs = [
    ...
    onnx_outputs = [
        ...
        ['det_pred_dicts_lidar_gop'],
    ]
    
    onnx_outputs_head = {
        'lidar-head': ['det_pred_dicts_lidar_cls', 'det_pred_dicts_lidar_box', 'det_pred_dicts_lidar_dir_cls'],
        'lidar-headgop': ['det_pred_dicts_lidar_gop_cls', 'det_pred_dicts_lidar_gop_box', 'det_pred_dicts_lidar_gop_dir_cls'],
        }
