...
if __name__ == '__main__':
    ...
    cfg_file = "/mnt/lustre/xuzhiyong/code/multimodal-3dgop-xuzhiyong/tools/onnx_utils/checkpoints/MM3DGOP-A02-V1.9-3heads_A02_3dgop_seg_det_2V_V1.9_No1/fusion_A02_3dgop_seg_det_2V_V1.9_No1_3heads_8m.yaml"
    checkpoint_path = "/mnt/lustre/xuzhiyong/code/multimodal-3dgop-xuzhiyong/tools/onnx_utils/checkpoints/MM3DGOP-A02-V1.9-3heads_A02_3dgop_seg_det_2V_V1.9_No1/checkpoint_epoch_17_3heads.pth"  # 在这里指定checkpoint_path
    save_onnx_root = "/mnt/lustre/xuzhiyong/code/multimodal-3dgop-xuzhiyong/tools/onnx_utils/onnx_output/tmp"
    batch_dict = {
            "vfe_input": torch.rand((70000, 9, 32, 1), dtype=dtype, device=device),  # lidar-vfe input
            ...
            "gridsample_ref_points": torch.rand(2, 3578, 1, 1, 15, 2, dtype=dtype, device=device),  # cam-atttransform ref_points input
        }
    module_list = [
        ['PillarVFE'], ['BaseBEVBackbone_FPN'], 
        ['DLANet_', 'CustomFPN'], ['AttentionTransform_Lidaraug'], ['BaseBEVBackbone_FPN_cam'], 
        ['pp_heavy_head', 'SegHead_pcseg'],  # lidar, 要和yaml文件中lidar, cam, fusion的顺序保持一致
        ['pp_heavy_head', 'SegHead_pcseg'],  # cam
        ['ConvFuser_later_atten', 'pp_heavy_head', 'SegHead_pcseg'],  # fusion
    ]
    onnx_inputs = [
        ['vfe_input'], ['spatial_features'], 
        ['camera_imgs'], ['gridsample_input', 'gridsample_ref_points'], ['cam_bev_backbone_input'], 
        ['spatial_features_2d_lidar'],
        ['spatial_features_2d_cam'],
        ['spatial_features_2d_lidar', 'spatial_features_2d_cam'],
    ]
    onnx_outputs = [
        ['vfe_output'], ['spatial_features_2d_lidar'], 
        ['image_fpn'], ['gridsample_output'], ['spatial_features_2d_cam'],
        ['det_pred_dicts_lidar', 'seg_pred_dicts_lidar'],
        ['det_pred_dicts_cam', 'seg_pred_dicts_cam'],
        ['det_pred_dicts_fusion', 'seg_pred_dicts_fusion'],
    ]
    
    onnx_outputs_head = {
        'lidar-head': ['det_pred_dicts_lidar_cls', 'det_pred_dicts_lidar_box', 'det_pred_dicts_lidar_dir_cls', 'seg_pred_dicts_lidar'],
        'cam-head': ['det_pred_dicts_cam_cls', 'det_pred_dicts_cam_box', 'det_pred_dicts_cam_dir_cls', 'seg_pred_dicts_cam'],
        'fuser-fusion-head': ['det_pred_dicts_fusion_cls', 'det_pred_dicts_fusion_box', 'det_pred_dicts_fusion_dir_cls', 'seg_pred_dicts_fusion'],
    }
    
    onnx_file_name = [
        'lidar-vfe', 'lidar-bev-backbone', 
        'cam-backbone', 'cam-atttransform', 'cam-bev-backbone', 
        'lidar-head', 
        'cam-head',
        'fuser-fusion-head', 
    ]
