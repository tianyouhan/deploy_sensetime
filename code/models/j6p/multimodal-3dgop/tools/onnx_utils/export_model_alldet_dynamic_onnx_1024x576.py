...
if __name__ == '__main__':
    ...
    export_fp16 = False
    export_dynamic = True  # adela部署需要静态维度
    dtype = torch.float16 if export_fp16 else torch.float32
    cfg_file = "/iag_ad_01/ad/zhanghongcheng/code/multimodal-3dgop/tools/onnx_utils/checkpoints/fusion_A02_3dgop_alldet_2V_V1.9.1_No1_1024x576_virtual/fusion_A02_3dgop_alldet_2V_V1.9_No1_3heads_8m_1024x576.yaml"
    checkpoint_path = "onnx_utils/checkpoints/fusion_A02_3dgop_alldet_2V_V1.9.1_No1_1024x576_virtual_0715/checkpoint_epoch_40_3heads.pth"  # 在这里指定checkpoint_path
    save_onnx_root = "/iag_ad_01/ad/zhanghongcheng/code/multimodal-3dgop/tools/onnx_utils/checkpoints/fusion_A02_3dgop_alldet_2V_V1.9.1_No1_1024x576_virtual_0715/"
    ...
    batch_dict = {
            ...
            "camera_imgs": torch.rand(2, 3, 576, 1024, dtype=dtype, device=device),  # cam-backbone input
            "center_camera_fov120": torch.rand(1, 3, 576, 1024, dtype=dtype, device=device),  # cam-backbone input
            "center_camera_fov30": torch.rand(1, 3, 576, 1024, dtype=dtype, device=device),  # cam-backbone input
            ...
            "gridsample_ref_points": torch.rand(2, 3578, 1, 1, 15, 2, dtype=dtype, device=device),  # 3578为fov120和fov30中投影到相机上点的max_len
            "gridsample_indexes_0": torch.rand((3578), dtype=dtype, device=device) * 1000,  # center_camera_fov120在3840中的index， 3578 有效pillar数
            "gridsample_indexes_1": torch.rand((3578), dtype=dtype, device=device) * 1000,  # center_camera_fov30在3840中的index
