...
CLS_MAPPING = {
    ...
    'VIHECLE_APPENDAGE': 'Truck', 
    'VEHICLE_AMBULANCE': 'Truck', 
    'VEHICLE_POLICE': 'Car',
    # 'VEHICLE_DOOR_REAR': 'Car',
    # 'VEHICLE_DOOR': 'Car',
    'PEDESTRIAN_TRAFFIC_POLICE': 'Pedestrian',  
    'VEHICLE_RUBBISH': 'Truck',
    'VEHICLE_MULTI_STAGE': 'Truck',
}
class GACBaiduDatasetPVBGOPVolc(DatasetTemplateSeg):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        root_path = (root_path if root_path is not None else Path(dataset_cfg.DATA_PATH))
        super().__init__(
            dataset_cfg=dataset_cfg, class_names=class_names, training=training, root_path=root_path, logger=logger
        )
        self.class_names_pvb = self.dataset_cfg.get('PVB_CLASS_NAMES', [])
        self.class_names_gop = self.dataset_cfg.get('GOP_CLASS_NAMES', [])
    ...
    def prepare_pp_heavy(self):
        ...
        # gop anchor generator
        anchor_cfg_gop = self.pp_heavy.TARGET_ASSIGNER.ANCHOR_GENERATOR_GOP
        anchor_generators_gop = []
        for a_cfg in anchor_cfg_gop:
            if a_cfg['type'] == "anchor_generator_stride":
                anchor_generator = AnchorGeneratorStride(
                    sizes=a_cfg['sizes'],
                    anchor_strides=a_cfg['strides'],
                ...
                raise Exception(f"\"{a_cfg[type]}\" is not a valid generator type!!")
            anchor_generators_gop.append(anchor_generator)
        
        self.target_assigner_gop = TargetAssigner(
            anchor_generators=anchor_generators_gop,
            pos_fraction=self.pp_heavy.TARGET_ASSIGNER.SAMPLE_POS_FRACTION,
            sample_size=self.pp_heavy.TARGET_ASSIGNER.SAMPLE_SIZE,
            region_similarity_fn_name=self.pp_heavy.TARGET_ASSIGNER.REGION_SIMILARITY_FN,
            box_coder=self.box_coder
        ...
        ret = self.target_assigner_gop.generate_anchors(feature_map_size)
        anchors_dict_gop = self.target_assigner_gop.generate_anchors_dict(feature_map_size)
        anchors_gop = ret["anchors"].reshape([-1, 7])
        anchors_ = {
            "anchors_gop": anchors_gop,
            "anchors_dict_gop": anchors_dict_gop,
        }  
        self.anchor_cache.update(anchors_)
        return
    ...
    def prepare_pp_heavy_gt(self, data_dict):
        ...
        if self.training:
            ...
            gt_names = np.array(self.class_names_pvb)[gt_classes-1]
        ...
        # gop
        anchors_gop = self.anchor_cache["anchors_gop"]
        anchors_dict_gop = self.anchor_cache["anchors_dict_gop"]
        data_dict["anchors_gop"] = anchors_gop
        anchors_mask_gop = None
        if self.training:
            # TODO
            gt_classes_gop = data_dict['gt_boxes_gop'][:,7].astype(np.int32)
            gt_names_gop = np.array(self.class_names_gop)[gt_classes_gop-1]
            targets_dict = self.target_assigner_gop.assign_v2(anchors_dict_gop, data_dict['gt_boxes_gop'][:,0:7], anchors_mask_gop,
                                                        gt_classes=gt_classes_gop, gt_names=gt_names_gop,
                                                        multihead=True)
            data_dict.update({
                'box_labels_gop': targets_dict['labels'],
                'reg_targets_gop': targets_dict['bbox_targets'],
                'reg_weights_gop': targets_dict['bbox_outside_weights'],
            })
            # data_dict['backward'] = np.array([0], dtype=np.int64)
        return data_dict
    ...
    def include_data(self, mode):
        ...
        info_sample_rate = self.dataset_cfg.get('INFO_SAMPLE_RATE', None)
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            if type(info_path) is not list:
                info_path = [info_path]
            if type(info_path) is list:
                infos = None
                for n, path in enumerate(info_path):
                    info = ceph_utils.ceph_read(path, dtype=None, use_ceph=self.use_ceph, client=self.ceph_client)
                    if info_sample_rate:
                        cur_rate = info_sample_rate[mode][n]
                        print(path, cur_rate)
                        if type(info) is list:
                            ...
                            print('dict')
                            keys = list(info.keys())[::cur_rate]  
                            info = {k: info[k] for k in keys}
                        else:
                            print('Warning! Not support pkl type')
                    self.logger.info(type(info))
                    self.logger.info(type(infos))
                    if infos is not None:
                        self.logger.info(len(infos))
                    if infos is None:
    ...
    def sample_with_split_rate(self, data, split, rate):
        """
        data: 原始list
        ...
        """
        result = []
        for (s, e), r in zip(split, rate):
            part = data[s:e]           # 切片
            sampled = part[::r]        # 按步长采样
            result.extend(sampled)     # 拼接
        return result
    
    def get_interval_data(self, cfg, cur_epoch=0):
        # import ipdb; ipdb.set_trace()
        print('infos_full', len(self.infos_full))
        if 'DATASET_INTERVAL' in cfg:
            rate = cfg.DATASET_INTERVAL['rate']
            split = cfg.DATASET_INTERVAL['split']
            
            self.infos = self.sample_with_split_rate(self.infos_full, split, rate)
            print(len(self.infos))
            self.infos_ref_frame = self.sample_with_split_rate(self.infos_ref_frame_full, split, rate)
            self.infos_pose = self.sample_with_split_rate(self.infos_pose_full, split, rate)
    ...
    def get_points_label_base(self, info, check_points=True):
        ...
        if check_points and points.shape[0] < 60000:  # 去除坏点云
            ...
            pkl_path = self.infos[index]
            res = ceph_utils.ceph_read(pkl_path, dtype=None, use_ceph=self.use_ceph, client=self.ceph_client)
            info = copy.deepcopy(res)
            return self.get_points_label_base(info, check_points)
    ...
    def get_points_label_info(self, index):
        ...
        if not isinstance(info, dict):
            print(f"{pkl_path} contains non-dict item: {type(info)}")
            raise TypeError("Expected a dict in pkl, but got something else.")
    ...
    def get_gt_box(self, input_dict, info):
        ...
        if data_source == 'pvb':
            dimensions = dimensions[:, [0, 2, 1]] # lhw > lwh
        else:
            dimensions = dimensions[:, [2, 0, 1]]  # whl > lwh
        # 限制yaw在[0, 2pi]
        rotation_y = common_utils.limit_period(rotation_y, offset=0.5, period=2*np.pi)
        if data_source == 'atx':
            rotation_y *= -1
        elif data_source == 'pvb':
            rotation_y += np.pi*0.5
        elif data_source == 'atxpvb':
            rotation_y *= -1
            # print("pvb yaw + pi/2")
        ...
        if 'FILTER_UNNORMAL_SIZE_BOXES' in self.box_cfg and self.box_cfg.FILTER_UNNORMAL_SIZE_BOXES[self.mode] and gt_boxes_lidar.shape[0] > 0:  # filter unnormal size boxes
            ...
            if mask is None:
                try:
                    mask = np.ones_like(annos['name'], dtype=np.bool_)
                except:
    ...
    def load_lidar_info(self, info):
        normalize_calib = self.lidar_config.get('NORM_CALIB', None)
        calib_path = info.get('calib_path', None)
        if calib_path is None:
            calib_path = '/'.join(info['velodyne_path'].split('/')[:-2]) + '/calib'
        if normalize_calib == "A02-J6M" or "EPI-MDC":
            extrinsic_file = 'front_lidar-to-car_center-extrinsic.json'
            lidar_extrinsic_path = os.path.join(calib_path, 'front_lidar', extrinsic_file)
            if self.use_ceph:
                lidar_extrinsic_path = self.mapping_path_to_ceph(lidar_extrinsic_path)
            extrinsic_calib = ceph_utils.ceph_read(lidar_extrinsic_path, None, self.use_ceph, self.ceph_client)
            key = [ele for ele in extrinsic_calib.keys()][0]
            lidar_extrinsic = np.array(
                extrinsic_calib[key]['param']['sensor_calib']['data'], dtype=np.float32)
        else:
    ...
    def update_gt_box(self, data_dict):
        ...
        gt_boxes = np.concatenate((data_dict['gt_boxes'], gt_classes.reshape(-1, 1).astype(np.float32)), axis=1).copy()
        mask_isgop = gt_boxes[:, 7] > len(self.class_names_pvb)
        mask_ispvb = gt_boxes[:, 7] <= len(self.class_names_pvb)
        assert np.all(mask_isgop == ~mask_ispvb), "mask_isgop 应该等于 mask_ispvb 的取反"
        # TODO num points in gt
        # print(mask_ispvb)
        # print(gt_boxes.shape)
        # pvb_gt_boxes = gt_boxes[mask_ispvb]
        # data_dict['gt_boxes'] = pvb_gt_boxes

        # gop_gt_boxes = gt_boxes[mask_isgop]
        # data_dict['gt_boxes_gop'] = gop_gt_boxes
        # data_dict['gt_boxes_gop'][:, 7] -= len(self.class_names_pvb)
        # data_dict['gt_names'] = data_dict['gt_names'][mask_ispvb]
        # data_dict['gt_names_gop'] = data_dict['gt_names'][mask_isgop]
        # data_dict['metadata']['annos_boxes'] = {
        #     'gt_names':data_dict['gt_names'],
        #     'gt_boxes': gt_boxes[mask_ispvb],
        #     'num_points_in_gt': data_dict['num_points_in_gt'][mask_ispvb],
        #     'gt_names_gop': data_dict['gt_names_gop'],
        #     'gt_boxes_gop': data_dict['gt_boxes_gop'],
        #     'num_points_in_gt_gop': data_dict['num_points_in_gt'][mask_isgop],
        #     }
        pvb_gt_boxes = gt_boxes[mask_ispvb]
        gop_gt_boxes = gt_boxes[mask_isgop]
        pvb_gt_names = np.array(data_dict['gt_names'])[mask_ispvb]
        gop_gt_names = np.array(data_dict['gt_names'])[mask_isgop]
        pvb_points = np.array(data_dict['num_points_in_gt'])[mask_ispvb]
        gop_points = np.array(data_dict['num_points_in_gt'])[mask_isgop]

        # GOP 类别 index 重编号
        gop_gt_boxes[:, 7] -= len(self.class_names_pvb)

        data_dict['gt_boxes'] = pvb_gt_boxes
        data_dict['gt_boxes_gop'] = gop_gt_boxes
        data_dict['gt_names'] = pvb_gt_names
        data_dict['gt_names_gop'] = gop_gt_names
        data_dict['metadata']['annos_boxes'] = {
            'gt_names': pvb_gt_names,
            'gt_boxes': pvb_gt_boxes,
            'num_points_in_gt': pvb_points,
            'gt_names_gop': gop_gt_names,
            'gt_boxes_gop': gop_gt_boxes,
            'num_points_in_gt_gop': gop_points,
        }
        data_dict['metadata']['annos_boxes_gop'] = {
            'gt_names': gop_gt_names,
            'gt_boxes': gop_gt_boxes,
            'num_points_in_gt': gop_points,
        }
    ...
    def __getitem__(self, index):
        ...
        data_dict.update({'data_source': data_source})
        # pvb weight
        if data_source in ['pvb', 'atxpvb', 'pvbgop']:
            data_source_weight = 1.0
        elif data_source in ['atx', 'A02']:
            data_source_weight = 0.0
        else:
            print(data_source[0])
            raise NotImplementedError
        data_dict.update({'data_source_weight': data_source_weight})
        # gop weight
        if data_source in ['pvb', 'atxpvb']:
            data_source_weight_gop = 0.0
        elif data_source in ['atx', 'A02', 'pvbgop']:
            data_source_weight_gop = 1.0
        else:
            print(data_source[0])
            raise NotImplementedError
        data_dict.update({'data_source_weight_gop': data_source_weight_gop})
        ...
        def kitti_eval(eval_det_annos, eval_gt_annos):
            ...
            if self.add_ignore:
                for i in range(len(eval_gt_annos)):
                    ...
                    try:
                        flag_keep_det = np.array([True]*(len(eval_det_annos[i]['name'])),dtype=np.bool_)
                    except:
            ...
            kitti_class_names_pvb = [map_name_to_kitti[x] for x in self.class_names_pvb]
            class_to_name_pvb = {
                    0: 'Car',
                    1: 'Pedestrian',
                    2: 'Cyclist',
            ...
            overlap_0_7 = np.array([[0.7, 0.5, 0.5, 0.7], [0.7, 0.5, 0.5, 0.7], [0.7, 0.5, 0.5, 0.7]])
            overlap_0_5 = np.array([[0.5, 0.25, 0.25, 0.5], [0.5, 0.25, 0.25, 0.5], [0.5, 0.25, 0.25, 0.5]])

            ap_result_str, ap_dict = kitti_eval.get_official_eval_result(
                gt_annos=eval_gt_annos, dt_annos=eval_det_annos, current_classes=kitti_class_names_pvb, 
                overlap_0_7=overlap_0_7,overlap_0_5=overlap_0_5, class_to_name=class_to_name_pvb, valid_classes=['Car', 'Pedestrian', 'Cyclist', 'Truck']
            )
    ...
    def evaluation_gop(self, det_annos, class_names, **kwargs):
        # if 'annos' not in self.infos[0].keys():
        #     return 'No ground-truth boxes for evaluation', {}
        ...
        def kitti_eval(eval_det_annos, eval_gt_annos):
            ...
            kitti_class_names = [map_name_to_kitti[x] for x in self.class_names_gop]
            ...
            # 注意gt_boxes中的id已经+1了，所以pred从name映射到id，也要从1开始
            kitti_class_names_gop = [map_name_to_kitti[x] for x in self.class_names_gop]
            class_to_name = {
                    0: 'DontCare',
                    1: 'cone',
                    2: 'pole',
            ...
            # import ipdb; ipdb.set_trace()
            ap_result_str, ap_dict = kitti_eval.get_official_eval_result(
                gt_annos=eval_gt_annos, dt_annos=eval_det_annos, current_classes=kitti_class_names_gop, 
        ...
        if not self.dataset_cfg.get('RANGE_EVAL', False):
            eval_det_annos = copy.deepcopy(det_annos)
            eval_gt_annos = [copy.deepcopy(gt_annos['metadata']['annos_boxes_gop']) for gt_annos in det_annos]
        else:
            eval_det_annos = []
            range1, range2 = kwargs['eval_range']
            for anno in det_annos:
    ...
    def generate_prediction_dicts(self, batch_dict, pred_dicts, class_names, output_path=None, head_mode=''):
        """
        Args:
            batch_dict:
                frame_id:
        ...
        def generate_single_sample_dict(box_dict, head_mode=''):
            pred_scores = box_dict[f'pred_scores{head_mode}'].cpu().numpy()
            pred_boxes = box_dict[f'pred_boxes{head_mode}'].cpu().numpy()
            pred_labels = box_dict[f'pred_labels{head_mode}'].cpu().numpy()
            pred_dict = get_template_prediction(pred_scores.shape[0])
            if pred_scores.shape[0] == 0:
                return pred_dict
            if 'gop' in head_mode:
                pred_dict['name'] = np.array(self.class_names_gop)[pred_labels - 1]
            else:
                pred_dict['name'] = np.array(self.class_names_pvb)[pred_labels - 1]
        ...
        for index, box_dict in enumerate(pred_dicts):
            single_pred_dict = generate_single_sample_dict(box_dict, head_mode)
