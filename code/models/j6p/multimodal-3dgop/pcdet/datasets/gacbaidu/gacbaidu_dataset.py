...
class GACBaiduDataset(DatasetTemplateSeg):
    ...
    def get_points_label_base(self, info, check_points=True):
        ...
        if type(info) == str:
            info = ceph_utils.ceph_read(info, dtype=None, use_ceph=True, client=self.ceph_client)
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        self.infos_0 = self.infos[0]
        if type(self.infos_0) == str:
            self.infos_0 = ceph_utils.ceph_read(self.infos_0, dtype=None, use_ceph=True, client=self.ceph_client)
        if 'annos' not in self.infos_0.keys():
