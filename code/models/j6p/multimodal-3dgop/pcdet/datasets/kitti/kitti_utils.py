...
def transform_3dgop_to_kitti_format(annos, map_name_to_kitti=None, info_with_fakelidar=False):
    """
    Args:
        annos:
        map_name_to_kitti: dict, map name to KITTI names (Car, Pedestrian, Cyclist)
    ...
    for anno in annos:
        ...
        for k in range(anno['name'].shape[0]):
            anno['name'][k] = map_name_to_kitti[anno['name'][k]] if type(anno['name'][k]) != np.ndarray else map_name_to_kitti[anno['name'][k][0]]

        anno['bbox'] = np.zeros((len(anno['name']), 4))
        anno['bbox'][:, 2:4] = 50  # [0, 0, 50, 50]
        anno['truncated'] = np.zeros(len(anno['name']))
        anno['occluded'] = np.zeros(len(anno['name']))
        gt_boxes = anno['gt_boxes'].copy() if 'gt_boxes' in anno else anno['pred_boxes'].copy() 

        if len(gt_boxes) > 0:
            if info_with_fakelidar:
                gt_boxes = box_utils.boxes3d_kitti_fakelidar_to_lidar(gt_boxes)

            anno['location'] = np.zeros((gt_boxes.shape[0], 3))
            anno['location'][:, 0] = -gt_boxes[:, 1]  # x = -y_lidar
            anno['location'][:, 1] = -gt_boxes[:, 2]  # y = -z_lidar
            anno['location'][:, 2] = gt_boxes[:, 0]  # z = x_lidar
            wlh = gt_boxes[:, 3:6]
            anno['dimensions'] = wlh[:, [1, 2, 0]]  # wlh ==> lhw
            anno['rotation_y'] = gt_boxes[:, 6]
            anno['alpha'] = -np.arctan2(-gt_boxes[:, 1], gt_boxes[:, 0]) + anno['rotation_y']
        else:
            anno['location'] = anno['dimensions'] = np.zeros((0, 3))
            anno['rotation_y'] = anno['alpha'] = np.zeros(0)
