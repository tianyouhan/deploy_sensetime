...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def build_graph(self, graph, cam=False):
        with graph.as_default():
            ...
            config = self.build_config() if not cam else self.build_config_cam()
            if cam:
                print("use cam metric")
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, distance_thresh=100, fake_gt_infos=True, cam=False):
        ...
        graph = tf.Graph()
        metrics = self.build_graph(graph, cam=cam)
