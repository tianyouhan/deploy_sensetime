...
import os
import math
class pp_heavy_head_pvbgop(nn.Module):
    def __init__(self,
                 cfg,
                 dataset,
                 num_class=2,
        ...
        """
        super(pp_heavy_head_pvbgop, self).__init__()
        self.cnt = 0
    ...
    def get_loss(self):
        ...
        tb_dict = {}
        # rpn head losses
        cls_preds = self.ret_dict[f'rpn_cls_preds{self.head_mode}'].float()
        box_preds = self.ret_dict[f'rpn_box_preds{self.head_mode}'].float() 
        dir_cls_preds = self.ret_dict[f'rpn_dir_cls_preds{self.head_mode}'].float() 
        iou_preds = self.ret_dict[f'rpn_iou_preds{self.head_mode}'] 
        var_preds = self.ret_dict[f'rpn_var_preds{self.head_mode}'].float() 
        ...
        if 'gop' in self.head_mode:
            data_source_weight = torch.tensor(self.data_source_weight_gop, device=loc_loss.device).view(batch_size, 1, 1)
        else:
            data_source_weight = torch.tensor(self.data_source_weight, device=loc_loss.device).view(batch_size, 1, 1)
        loc_loss_w = data_source_weight * loc_loss
        cls_loss_w = data_source_weight * cls_loss
        loss_weights_dict = cfg.RPN_STAGE.RPN_HEAD.LOSS_WEIGHTS
        loc_loss_reduced = loc_loss_w.sum() / batch_size
        loc_loss_reduced_total = loc_loss_reduced * loss_weights_dict['loc_weight']
        cls_loss_reduced = cls_loss_w.sum() / batch_size
        ...
        tb_dict[f'rpn_loss_loc{self.head_mode}'] = loc_loss_reduced_total.item()
        tb_dict[f'rpn_loss_cls{self.head_mode}'] = cls_loss_reduced_total.item()
        ...
        if cfg.RPN_STAGE.RPN_HEAD.USE_DIRECTION_CLASSIFIER:
            ...
            rpn_loss += dir_loss
            tb_dict[f"rpn_dir_loss_reduced{self.head_mode}"] = dir_loss.item()

        tb_dict[f'rpn_loss{self.head_mode}'] = rpn_loss.item()
    ...
    def limit_period_torch(self, val, offset=0.5, period=2*torch.pi):
        """将角度限制到某个周期范围内"""
        return val - torch.floor(val / period + offset) * period

    def normalize_yaw_torch(self, arr, offset=0.5, period=2*torch.pi):
        """
        对 (N,7) Tensor 的第7维 yaw 进行限制
        arr: torch.Tensor, shape (N,7)
        """
        arr = arr.clone()  # 避免修改原始 tensor
        arr[:, 6] = self.limit_period_torch(arr[:, 6], offset=offset, period=period)
        return arr
    
    def predict_rpn(self, input_dict, preds_dict):
        cfg = self.cfg
        batch_size = input_dict[f'anchors{self.head_mode}'].shape[0]
        batch_anchors = input_dict[f"anchors{self.head_mode}"].view(batch_size, -1, 7)
        if "anchors_mask" not in input_dict:
            batch_anchors_mask = [None] * batch_size
        else:
            batch_anchors_mask = input_dict[f"anchors_mask{self.head_mode}"].view(batch_size, -1)
        # TODO
        ...
        batch_box_preds = preds_dict[f"rpn_box_preds{self.head_mode}"].view(batch_size, -1, self.box_coder.code_size)
        batch_cls_preds = preds_dict[f"rpn_cls_preds{self.head_mode}"].view(batch_size, -1, num_class_with_bg).float()
        ...
        if os.getenv("CALIB") == 'True':
            task_folder = f"{self.task}-decode_box_pred/"
            calib_path = os.getenv("CALIB_PATH")
            save_dir = os.path.join(calib_path, task_folder)
            save_np(os.path.join(save_dir, "outputs/decode_{}_box{}/{}".format(self.task, self.head_mode, self.cnt)), batch_box_preds)
            
        if cfg.RPN_STAGE.RPN_HEAD.USE_DIRECTION_CLASSIFIER:
            batch_dir_preds = preds_dict[f"rpn_dir_cls_preds{self.head_mode}"].view(batch_size, -1, 2)
        else:
        ...
        if cfg.RPN_STAGE.RPN_HEAD.REG_VARIANCE.OUTPUT:
            batch_var_preds = preds_dict[f'rpn_var_preds{self.head_mode}'].view(batch_size, -1, self.box_coder.code_size)
        ...
        for box_preds, cls_preds, dir_preds, iou_preds, var_preds, a_mask in zip(
            ...
            thresh_list = [torch.tensor([cfg.TEST.SCORE_THRESH_LIST[k]], device=rank_scores.device).type_as(rank_scores) for k in range(len(cfg.TEST.SCORE_THRESH_LIST))]
            ...
            if cfg.FPGA_NMS:
                ...
                for k in range(len(cfg.CLASS_NAMES)):
                    ...
                    boxes_for_nms_ = boxes_for_nms.clone()
                    boxes_for_nms_[:, 6] *= -1
            ...
            if selected_boxes.shape[0] != 0:
                ...
                # print('limit yaw')
                selected_boxes_yaw = self.normalize_yaw_torch(selected_boxes)
                final_box_preds = selected_boxes_yaw
                # import ipdb; ipdb.set_trace()
                ...
                # predictions
                predictions_dict = {
                    f"pred_boxes{self.head_mode}": final_box_preds,
                    f"pred_scores{self.head_mode}": final_scores,
                    f"pred_labels{self.head_mode}": final_labels + 1,  # pcdet格式要求预测结果从1开始
                }
                if cfg.RPN_STAGE.RPN_HEAD.REG_VARIANCE.OUTPUT:
                    predictions_dict[f"var{self.head_mode}"] = selected_var
            else:
                dtype = batch_box_preds.dtype
                device = batch_box_preds.device
                predictions_dict = {
                    f"pred_boxes{self.head_mode}": torch.zeros([0, 7], dtype=dtype, device=device),
                    f"pred_scores{self.head_mode}": torch.zeros([0], dtype=dtype, device=device),
                    f"pred_labels{self.head_mode}": torch.zeros([0, 4], dtype=top_labels.dtype, device=device),
                }
                if cfg.RPN_STAGE.RPN_HEAD.REG_VARIANCE.OUTPUT:
    ...
    def forward(self, data_dict):
        ...
        ret = {
            f"box_preds{self.head_mode}": torch.cat([ret_dict["box_preds"] for ret_dict in ret_dicts], dim=1),
            f"cls_preds{self.head_mode}": torch.cat([ret_dict["cls_preds"] for ret_dict in ret_dicts], dim=1),
        }
        if self._use_direction_classifier:
            ret[f"dir_cls_preds{self.head_mode}"] = torch.cat([ret_dict["dir_cls_preds"] for ret_dict in ret_dicts], dim=1)
        if cfg.RPN_STAGE.RPN_HEAD.REG_VARIANCE.OUTPUT:
            ret[f"var_preds{self.head_mode}"] = torch.cat([ret_dict["var_preds"] for ret_dict in ret_dicts], dim=1)

        # return ret, distiller_res
        rpn_box_preds = ret[f'box_preds{self.head_mode}']
        rpn_cls_preds = ret[f'cls_preds{self.head_mode}']
        rpn_dir_cls_preds = ret[f'dir_cls_preds{self.head_mode}'] if f'dir_cls_preds{self.head_mode}' in ret else None
        rpn_iou_preds = ret[f'iou_preds{self.head_mode}'] if f'iou_preds{self.head_mode}' in ret else None
        rpn_var_preds = ret[f'var_preds{self.head_mode}'] if f'var_preds{self.head_mode}' in ret else None
        self.ret_dict = {}
        self.ret_dict[f'rpn_cls_preds{self.head_mode}'] = rpn_cls_preds
        self.ret_dict[f'rpn_box_preds{self.head_mode}'] = rpn_box_preds
        self.ret_dict[f'rpn_dir_cls_preds{self.head_mode}'] = rpn_dir_cls_preds
        self.ret_dict[f'rpn_iou_preds{self.head_mode}'] = rpn_iou_preds
        self.ret_dict[f'rpn_var_preds{self.head_mode}'] = rpn_var_preds
        ...
        if os.getenv("CALIB") == 'True':
            ...
            if os.getenv("det_cls_with_sigmoid") == 'True':
                save_np(os.path.join(save_dir, "outputs/det_pred_dicts_{}_cls{}/{}".format(self.task, self.head_mode, self.cnt)), torch.sigmoid(rpn_cls_preds.unsqueeze(0)))
            else:
                save_np(os.path.join(save_dir, "outputs/det_pred_dicts_{}_cls{}/{}".format(self.task, self.head_mode, self.cnt)), rpn_cls_preds.unsqueeze(0))
            save_np(os.path.join(save_dir, "outputs/det_pred_dicts_{}_box{}/{}".format(self.task, self.head_mode, self.cnt)), rpn_box_preds.unsqueeze(0))
            save_np(os.path.join(save_dir, "outputs/det_pred_dicts_{}_dir_cls{}/{}".format(self.task, self.head_mode, self.cnt)), rpn_dir_cls_preds.unsqueeze(0))

        #
        if self.training:
            self.labels = data_dict[f'box_labels{self.head_mode}']  # -1为背景，类别从0开始
            self.reg_targets = data_dict[f'reg_targets{self.head_mode}']
            self.anchors = data_dict[f'anchors{self.head_mode}']
            self.data_source = data_dict['data_source']
            self.data_source_weight = data_dict['data_source_weight']
            self.data_source_weight_gop = data_dict['data_source_weight_gop']
        else:
            if self.cfg.get('ONNX_FORWARD', False):
                import ipdb; ipdb.set_trace()
                self.ret_dict.pop(f'rpn_iou_preds{self.head_mode}')
                self.ret_dict.pop(f'rpn_var_preds{self.head_mode}')
                self.ret_dict[f'rpn_cls_preds{self.head_mode}'] = rpn_cls_preds.view(1, rpn_cls_preds.shape)
                self.ret_dict[f'rpn_box_preds{self.head_mode}'] = rpn_box_preds
                self.ret_dict[f'rpn_dir_cls_preds{self.head_mode}'] = rpn_dir_cls_preds
                data_dict[f'det_pred_dicts_{self.task}{self.head_mode}'] = self.ret_dict
            else:
                data_dict_task = {}
                pred_dicts = self.predict_rpn(data_dict, self.ret_dict)
                data_dict_task[f'final_box_dicts{self.head_mode}'] = pred_dicts
                if self.task is not None:
                    data_dict[f'det_pred_dicts_{self.task}{self.head_mode}'] = data_dict_task
                else:
    ...
    def forward_onnx(self, data_dict):
        ...
        self.ret_dict = {}
        self.ret_dict[f'rpn_cls_preds{self.head_mode}'] = rpn_cls_preds.unsqueeze(0)
        # self.ret_dict['rpn_cls_preds'] = torch.sigmoid(rpn_cls_preds.unsqueeze(0))
        self.ret_dict[f'rpn_box_preds{self.head_mode}'] = rpn_box_preds.unsqueeze(0)
        self.ret_dict[f'rpn_dir_cls_preds{self.head_mode}'] = rpn_dir_cls_preds.unsqueeze(0)
