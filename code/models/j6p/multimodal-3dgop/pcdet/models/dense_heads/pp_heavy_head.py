...
class pp_heavy_head(nn.Module):
    def __init__(self,
        ...
        """
        super(pp_heavy_head, self).__init__()
        self.cnt = 0
    ...
    def predict_rpn(self, input_dict, preds_dict):
        ...
        if os.getenv("CALIB") == 'True':
            ...
            # import ipdb; ipdb.set_trace()
            save_np(os.path.join(save_dir, "outputs/decode_{}_box/{}".format(self.task, self.cnt)), batch_box_preds)
        ...
        for box_preds, cls_preds, dir_preds, iou_preds, var_preds, a_mask in zip(
            ...
            top_scores_keep = top_scores_keep.bool()
