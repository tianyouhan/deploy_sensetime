...
class AttentionTransform_Lidaraug(nn.Module):
    """
    ...
    def __init__(self, model_cfg):
        ...
        self.model_type = self.model_cfg.get('MODEL_TYPE', None)
        self.add_lidar2image_noise = self.model_cfg.get('LIDAR2IMAGE_NOISE', None)
        self.init_layers()
        self.init_weights()
    ...
    def point_sampling(self, reference_points, lidar2img, img_aug_matrix, lidar_aug_matrix):     
        # lidar2img (B, N, 4, 4)
        ...
        img_aug_matrix = img_aug_matrix.view(
            1, B, num_cam, 1, 4, 4).repeat(D, 1, 1, num_query, 1, 1)
        lidar_aug_matrix = lidar_aug_matrix.view(
            1, B, 1, 1, 4, 4).repeat(D, 1, num_cam, num_query, 1, 1)
        
        reference_points = torch.matmul(lidar_aug_matrix.to(torch.float32), 
    ...
    def forward_onnx(self, batch_dict):
        ...
        '''
        if 'gridsample_input' in batch_dict:
            feat = batch_dict['gridsample_input']
        else:
            feat = batch_dict['image_fpn'][0]
        reference_points = batch_dict['gridsample_ref_points']
        feat_flatten = []
        spatial_shapes = []
        mlvl_feats = [feat]
        ...
        if os.getenv("ONNX_BRANCH") == "True":
            if os.getenv("DYNAMIC_ONNX") == "True":
                indexes_0 = batch_dict['gridsample_indexes_0'].type(torch.long)  # (N)
                indexes_1 = batch_dict['gridsample_indexes_1'].type(torch.long)  # (M)
                indexes = [indexes_0, indexes_1]
                ref_points_valid_num = batch_dict['ref_points_valid_num']  # (1, 3840)
            else:
                indexes = batch_dict['gridsample_indexes'].type(torch.long)  # (2, 3840)
                valid_flag = batch_dict['gridsample_indexes_valid_flag'] > 0  # (2, 3840)
                ref_points_valid_num = batch_dict['ref_points_valid_num']  # (1, 3840)
                indexes = [indexes[i][valid_flag[i]] for i in range(self.num_views)]

            slots = torch.zeros(bs, ref_points_valid_num.shape[1], self.out_channel, dtype=mlvl_feats[0].dtype, device=mlvl_feats[0].device)
            for i in range(self.num_views):
                slots[0, indexes[i]] += output[0, i, :indexes[i].shape[0]]
            slots = slots / ref_points_valid_num[..., None]
            batch_dict['cam_bev_backbone_input'] = slots
        else:
            batch_dict['gridsample_output'] = output
    ...
    def forward(self, batch_dict):
        ...
        """
        if self.model_type is not None and self.model_type=='STU':
            mlvl_feats = batch_dict['image_fpn_stu']
        else:
        ...
        if os.getenv("CALIB") == 'True':
            ...
            save_dir = os.path.join(calib_path, 'cam-atttransform')
            save_np(os.path.join(save_dir, "inputs/gridsample_input/{}".format(self.cnt)), mlvl_feats[0])
        ...
        if self.training and self.add_lidar2image_noise is not None:
            prob = self.add_lidar2image_noise.NOISE_PROB
            size = self.add_lidar2image_noise.NOISE_SIZE
            noise = (lidar2image[:,:,0:3,0:4] / size).to(lidar2image.device)
            if np.random.random() < prob:
                if np.random.randint(2):
                    noise *= -1
                lidar2image[:,:,0:3,0:4] += noise
        ...
        if os.getenv("CALIB") == 'True':
            ...
            for idx in range(self.num_views):
                save_np(os.path.join(save_dir, "calib/camera_intrinsics/camera_{}/{}".format(idx, self.cnt)), img_aug_matrix.squeeze()[idx] @ batch_dict['camera_intrinsics'].squeeze()[idx])
                save_np(os.path.join(save_dir, "calib/lidar2camera/camera_{}/{}".format(idx, self.cnt)), batch_dict['lidar2camera'].squeeze()[idx])
                save_np(os.path.join(save_dir, "calib/camera2lidar/camera_{}/{}".format(idx, self.cnt)), batch_dict['camera2lidar'].squeeze()[idx])
        
        reference_points_cam, bev_mask = self.point_sampling(ref_3d, lidar2image, img_aug_matrix, lidar_aug_matrix)
        ...
        if os.getenv("CALIB") == 'True':
            calib_path = os.getenv("CALIB_PATH")
            save_dir = os.path.join(calib_path, 'cam-atttransform')
            save_np(os.path.join(save_dir, "inputs/gridsample_ref_points/{}".format(self.cnt)), reference_points[:, :, None, None, :, :])
        ...
        if os.getenv("CALIB") == 'True':
            calib_path = os.getenv("CALIB_PATH")
            save_dir = os.path.join(calib_path, 'cam-atttransform')
            save_np(os.path.join(save_dir, "outputs/gridsample_output/{}".format(self.cnt)), output)
        ...
        if os.getenv("CALIB") == 'True':
            ...
            save_np(os.path.join(save_dir, "inputs/gridsample_ref_points/{}".format(self.cnt)), reference_points[:, :, None, None, :, :])
            save_np(os.path.join(save_dir, "inputs/gridsample_indexes_0/{}".format(self.cnt)), indexes[0][0])
            save_np(os.path.join(save_dir, "inputs/gridsample_indexes_1/{}".format(self.cnt)), indexes[1][0])
            save_np(os.path.join(save_dir, "inputs/ref_points_valid_num/{}".format(self.cnt)), count)

        if os.getenv("CALIB") == 'True':
            calib_path = os.getenv("CALIB_PATH")
            save_dir = os.path.join(calib_path, 'cam-bev-backbone')
            save_np(os.path.join(save_dir, "inputs/cam_bev_backbone_input/{}".format(self.cnt)), slots.unsqueeze(0))
        ...
        if self.model_type is not None and self.model_type=='STU':
            batch_dict['spatial_features_img_stu'] = slots
        else:
