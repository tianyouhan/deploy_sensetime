...
class BevFusion_PVBGOP_Qat(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        if model_cfg.get('FUSE_LATER', False):
            self.module_topology = [
                ...
                'dense_head_det_lidar_gop', 'dense_head_det_cam_gop', 'dense_head_det_fusion_gop', 
                'point_head', 'roi_head'
            ]
        else:
            self.module_topology = [
    ...
    def dense_head_base(self, model_info_dict, head_name=None):
        ...
        if 'pp_heavy' not in head_cfg.NAME:
            ...
            dense_head_module = dense_heads.__all__[head_cfg.NAME](
                ...
                num_class=cls_nums if not head_cfg.CLASS_AGNOSTIC else 1,
                head_mode=head_mode,
                )
    ...
    def build_dense_head_det_multitask(self):
        for module_name in ['dense_head_det_lidar', 'dense_head_det_cam', 'dense_head_det_fusion', 'dense_head_det_lidar_gop', 'dense_head_det_cam_gop', 'dense_head_det_fusion_gop']:
            setattr(self, f'build_{module_name}', lambda model_info_dict, head_name=module_name.upper(): self.dense_head_base(model_info_dict, head_name))
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            ...
            for module in ['DENSE_HEAD_DET', 'DENSE_HEAD_DET_FUSION', 'DENSE_HEAD_DET_LIDAR', 'DENSE_HEAD_DET_CAM', 'DENSE_HEAD_DET_LIDAR_GOP', 'DENSE_HEAD_DET_CAM_GOP', 'DENSE_HEAD_DET_FUSION_GOP']:
                if self.model_cfg.get(module, None) is not None:
                    module = module.lower()
                    head_loss_weight = self.model_cfg.get('HEAD_LOSS_WEIGHT', 1.0)
                    task = module.split('_')[-1]
                    if task == 'gop':
                        task = module.split('_')[-2].lower() + '_gop'
                    loss_, tb_dict_ = getattr(self, module).get_loss()
                    tb_dict_ = {f'loss_{module}': {f'loss_{task}': loss_.item(), **tb_dict_}}
                    loss += head_loss_weight * loss_
            ...
            for module in ['DENSE_HEAD_DET', 'DENSE_HEAD_DET_FUSION', 'DENSE_HEAD_DET_LIDAR', 'DENSE_HEAD_DET_CAM', 'DENSE_HEAD_DET_LIDAR_GOP', 'DENSE_HEAD_DET_CAM_GOP', 'DENSE_HEAD_DET_FUSION_GOP']:
                if self.model_cfg.get(module, None) is not None:
                    ...
                    if task in ['lidar', 'cam', 'fusion']: 
                        ...
                        _task = f'_{task}'
                    elif task in ['lidar_gop', 'cam_gop', 'fusion_gop']: 
                        # TODO
                        det_pred_dict, det_recall_dict = self.post_processing(batch_dict, task_key=f'det_pred_dicts_{task}')
                        _task = f'_{task}'
                    else:
                    ...
                    if os.getenv("CALIB") == 'True':
                        ...
                        for key in det_pred_dict[0]:
                            if not key.startswith("pred_"):
                                continue
                            save_np(os.path.join(save_dir, "det/{}/{}/{}".format(task, key, self.cnt)), det_pred_dict[0][key])
    ...
    def forward_onnx(self, batch_dict, onnx_module=None, onnx_outputs=None, task=None):
        for cur_module in self.module_list:
            ...
            if cur_module_name in onnx_module:
                ...
                if cur_module_name in ['SegHead_pcseg', 'pp_heavy_head', 'pp_heavy_head_pvbgop']:  # 根据task区分不同head
                    if not (task is not None and cur_module.task == task):
                        # import ipdb; ipdb.set_trace()
                        continue
                
                print(f'{cur_module_name} forward')
                if cur_module_name in ['AttentionTransform', 'AttentionTransform_Lidaraug', 'PillarVFEQat', 'PointPillarScatter_Seg_Qat', 'BaseBEVBackbone_FPN_cam', 'BaseBEVBackbone_cam', \
                                       'ResNet_bev', 'DLANet', 'DLANet_', 'pp_heavy_head_qat', 'pp_heavy_head_pvbgop_qat']:
    ...
    def forward_qat(self, batch_dict):
        for cur_module in self.module_list:
            cur_module_name = type(cur_module).__name__
            if cur_module_name in ['PillarVFEQat', 'PointPillarScatter_Seg_Qat']:
                batch_dict = cur_module.forward_qat(batch_dict)
            else:
                batch_dict = cur_module(batch_dict)
    ...
    def forward_qat_onnx(self, batch_dict, onnx_module=None, onnx_outputs=None, task=None):
        for cur_module in self.module_list:
            cur_module_name = type(cur_module).__name__
            if cur_module_name in onnx_module:
                ...
                if cur_module_name in ['AttentionTransform', 'AttentionTransform_Lidaraug', 'PillarVFEQat', 'PointPillarScatter_Seg_Qat', 'BaseBEVBackbone_FPN_cam', 'BaseBEVBackbone_cam', \
                    ...
                    if cur_module_name in ['BaseBEVBackbone_FPN_cam', 'BaseBEVBackbone_cam']:
                        batch_dict = self.forward_onnx_bev_cam(batch_dict, cur_module)
                    elif cur_module_name in ['PillarVFEQat', 'PointPillarScatter_Seg_Qat']:
                        batch_dict = cur_module.forward_qat_onnx(batch_dict)
                    else:
                        batch_dict = cur_module.forward_onnx(batch_dict)
                else:
                    batch_dict = cur_module(batch_dict)
    ...
    def post_processing(self, batch_dict, task_key=None):
        ...
        if task_key is None:
            ...
            if 'gop' in task_key:
                ...
                head_mode = ''
            final_pred_dict = batch_dict[task_key][f'final_box_dicts{head_mode}']
        recall_dict = {}
        for index in range(batch_size):
            pred_boxes = final_pred_dict[index][f'pred_boxes{head_mode}']
