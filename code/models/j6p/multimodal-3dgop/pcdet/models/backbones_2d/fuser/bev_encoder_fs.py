...
class BEV_Encoder_fs(nn.Module):
    def __init__(self,model_cfg) -> None:
        super().__init__()
        self.model_cfg = model_cfg
        self.numC_input = self.model_cfg.NUMC_INPUT
        self.num_channels = self.model_cfg.NUM_CHANNELS
        self.in_channels = self.model_cfg.IN_CHANNEL_EN
        self.out_channels = self.model_cfg.OUT_CHANNEL_EN
        ...
        fs_in_channel = self.model_cfg.IN_CHANNEL
        fs_out_channel = self.model_cfg.OUT_CHANNEL
        self.conv = nn.Sequential(
            nn.Conv2d(fs_in_channel, fs_out_channel, 3, padding=1, bias=False),
            nn.BatchNorm2d(fs_out_channel),
            nn.ReLU(True)
            )
    def forward(self,batch_dict):
        ...
        """
        x = batch_dict['spatial_features_img']
        x = self.img_bev_encoder_backbone(x)
        x1 = self.img_bev_encoder_neck(x)
        lidar_bev = batch_dict['spatial_features']
        # print("x1", x1.shape)
        # print("lidar_bev ",lidar_bev.shape)
        img_bev = F.interpolate(x1, size=lidar_bev.shape[-2:], mode='bilinear', align_corners=False)
