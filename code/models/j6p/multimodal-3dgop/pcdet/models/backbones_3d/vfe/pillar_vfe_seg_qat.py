...
class PFNLayerLinear(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 use_norm=True,
...
class PillarVFEQat(nn.Module):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        ...
        self.pfn_layer_type = self.model_cfg.get("PFN_LAYER_TYPE", "Conv")
        ...
        for i in range(len(num_filters)-1):
            ...
            if i < len(num_filters) - 2:
                ...
                last_layer = True
            if self.pfn_layer_type == "Conv":
                pfn_layers.append(
                    PFNLayer(in_filters, out_filters, self.use_norm, last_layer=last_layer)
                )
            elif self.pfn_layer_type == "Linear":
                pfn_layers.append(
                    PFNLayerLinear(in_filters, out_filters, self.use_norm, last_layer=last_layer)
                )
            else:
                raise ValueError("{} is not support yet.".format(self.pfn_layer_type))
        ...
        self.quan_vfe_input = QuantStub(1/128.0)
    ...
    def init_weights(self):
        ...
        for key, val in state_dict.items():
            key = key.replace('vfe.', '')
            if 'linear' in key and self.pfn_layer_type == "Conv":
    ...
    def forward_qat(self, batch_dict, **kwargs):
        ...
        batch_dict = self.vfe_input_prepare(batch_dict)
        features = batch_dict['vfe_input']
        features = self.quan_vfe_input(features)
        features = features.permute(0, 2, 1, 3).squeeze(-1)
