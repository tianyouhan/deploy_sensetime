...
def decode_bbox_from_heatmap(heatmap, rot_cos, rot_sin, center, center_z, dim,
    ...
    if score_thresh is not None:
        if not isinstance(score_thresh, list):
            mask &= (final_scores > score_thresh)
        else:
            score_thresh = torch.tensor(score_thresh).to(final_scores.device)
            scores_cls = score_thresh[final_class_ids.long()]
            mask &= (final_scores > scores_cls)
