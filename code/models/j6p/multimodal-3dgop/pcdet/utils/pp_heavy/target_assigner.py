...
class AnchorGeneratorStride:
    def __init__(self,
        ...
        self._feature_map_scale_factor = feature_map_scale_factor
    ...
    def generate(self, feature_map_size):
        # feature_map_size: z, y, x
        if feature_map_size[0] != 1:
            feature_map_size[0] = int(feature_map_size[0]*self._feature_map_scale_factor)
        feature_map_size[1] = int(feature_map_size[1]*self._feature_map_scale_factor)
        feature_map_size[2] = int(feature_map_size[2]*self._feature_map_scale_factor)
...
class TargetAssigner(object):
    ...
    def generate_anchors(self, feature_map_size):
        ...
        anchors = np.concatenate(anchors_list, axis=0)  # axis=-2
    ...
    def generate_anchors_dict(self, feature_map_size):
        ...
        for anchor_generator, match_thresh, unmatch_thresh in zip(self.anchor_generators,
            ...
            if anchors_dict[class_name] == {}:
                anchors_dict[class_name]["anchors"] = anchors.reshape([-1, 7])
                anchors_dict[class_name]["matched_thresholds"] = match_list[-1]
                anchors_dict[class_name]["unmatched_thresholds"] = unmatch_list[-1]
            else:
                anchors_dict[class_name]["anchors"] = np.concatenate([anchors_dict[class_name]["anchors"],anchors.reshape([-1, 7])], axis=0)
                anchors_dict[class_name]["matched_thresholds"] = np.concatenate([anchors_dict[class_name]["matched_thresholds"],match_list[-1]], axis=0)
                anchors_dict[class_name]["unmatched_thresholds"] = np.concatenate([anchors_dict[class_name]["unmatched_thresholds"],unmatch_list[-1]], axis=0)
        return anchors_dict
    ...
    def nearest_iou_similarity(boxes1, boxes2):
        ...
        boxes1_bv = box_np_ops.rbbox2d_to_near_bbox(boxes1_rbv)
        boxes2_bv = box_np_ops.rbbox2d_to_near_bbox(boxes2_rbv)
        ret = box_np_ops.iou_jit(boxes1_bv, boxes2_bv, eps=0.0)
        return ret

    @staticmethod
    def rotated_iou_similarity(boxes1, boxes2):
        import pcdet.ops.iou3d_nms.iou3d_nms_utils as iou3d_utils
        boxes1_cuda = torch.cuda.FloatTensor(boxes1)
        boxes2_cuda = torch.cuda.FloatTensor(boxes2)
        ret = iou3d_utils.boxes_iou_bev_lidar(boxes1_cuda, boxes2_cuda).cpu().numpy()
        return ret
    ...
    def assign_v2(self,
        ...
        def similarity_fn(anchors, gt_boxes):
            return self.region_similarity_calculator(anchors, gt_boxes)
        ...
        for class_name, anchor_dict in anchors_dict.items():
            # if class_name.startswith("Truck"):
            #     class_name = "Truck"
        ...
        if not multihead:
            ...
            targets_dict["bbox_targets"] = np.concatenate([v for v in targets_dict["bbox_targets"]], axis=0)
            targets_dict["labels"] = np.concatenate([v for v in targets_dict["labels"]], axis=0)
            targets_dict["bbox_outside_weights"] = np.concatenate([v for v in targets_dict["bbox_outside_weights"]], axis=0)
        return targets_dict
