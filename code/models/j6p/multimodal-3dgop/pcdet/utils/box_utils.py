...
def boxes_normalize(boxes3d, is_bottom_center=False, is_clockwise=False):
    """
    boxes define to pcdet format (box_center, anticlockwise)
    """
    boxes3d_norm = boxes3d.copy()
    if is_bottom_center:
        boxes3d_norm[:, 2] = boxes3d_norm[:, 2] + boxes3d_norm[:, 5] / 2.0
    if is_clockwise:
        boxes3d_norm[:, 6] = -boxes3d_norm[:, 6]
    return boxes3d_norm
