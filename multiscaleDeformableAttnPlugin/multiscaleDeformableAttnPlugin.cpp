...
static char const* DMHA_NAME{"TEST_TRT"};
} // namespace
...
// bool MultiscaleDeformableAttnPlugin::supportsFormatCombination(
//     int32_t pos, nvinfer1::PluginTensorDesc const* ioDesc, int32_t nbInputs, int32_t nbOutputs) PLUGIN_NOEXCEPT
// {
//   bool ret = false;
//   // printf("ioDesc[0].type = %d, ioDesc[0].format = %d\n", ioDesc[0].type, ioDesc[0].format);
//   const auto& desc = ioDesc[pos];
//   // printf("desc.type = %d, desc.format = %d\n", desc.type, desc.format);
//   // printf("[supportsFormatCombination] pos = %d, type = %d, format = %d\n", pos, desc.type, desc.format);
//   // printf("DataType::kINT32 = %d, DataType::kINT8 = %d, type = %d\n",
//   //      nvinfer1::DataType::kINT32, nvinfer1::DataType::kINT8, ioDesc[pos].type);
//   if (ioDesc[pos].format == nvinfer1::TensorFormat::kLINEAR) {
//     if ((pos == 1) || (pos == 2)) {
//       ret = (ioDesc[pos].type == nvinfer1::DataType::kINT32 ||
//             ioDesc[pos].type == nvinfer1::DataType::kINT8 ||
//             desc.type == nvinfer1::DataType::kINT64);
//     } else {
//       ret = ((ioDesc[pos].type == ioDesc[0].type) &&
